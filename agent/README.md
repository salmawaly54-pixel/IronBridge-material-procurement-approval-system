...
## `agent.py` -- interactive, Claude-driven
...
decide which to call and with what arguments. Nothing in this file
...
| Sampling | `make_sampling_callback` calls the Anthropic API directly from *this* process -- the client's own model, never the server's. |
